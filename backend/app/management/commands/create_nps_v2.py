...
class DailyMealPlanGenerator:
    ...
    def __init__(self):
        self.user_weight = 80
        self.user_energy_intake = 2200
        self.meal_cache = {}
        self.meal_counts = defaultdict(int)  # Track meal occurrences in heap

    def _get_meals_by_type(self, meal_type: str) -> List[Meal]:
        """Get meals with caching and sampling"""
        if meal_type not in self.meal_cache:
            queryset = Meal.objects.filter(Type=meal_type, Country="Ireland").only(
                'id', 'Total_Energy', 'Total_Protein',
                'Total_Fat', 'Total_Carbs', 'Total_Fibre', 'Total_Calcium', 
                'Total_Iron', 'Total_Folate', 'Food_Groups_Counter',
            )
            self.meal_cache[meal_type] = sample(list(queryset), min(SAMPLE_SIZE, queryset.count()))
        return self.meal_cache[meal_type]

    def generate_combinations(self) -> Iterator[Tuple[Meal]]:
        """Generate meal combinations with smart sampling"""
        meal_sets = [
            self._get_meals_by_type("Breakfast"),
            self._get_meals_by_type("Snack"),
            self._get_meals_by_type("Lunch"),
            self._get_meals_by_type("Snack"),  # Afternoon snack
            self._get_meals_by_type("Dinner")
        ]
        return product(*meal_sets)
